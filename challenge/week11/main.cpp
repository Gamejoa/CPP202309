...
void checkState(int map[][mapX], int user_x, int user_y, User& user); // 현재 상태 체크 함수  (추가한 부분)


void drinkPotion(User& user) {
	user.IncreaseHp(2);
...
int main() {
	...
	// 게임 시작 
	while (CheckUser(my_user)) { // 사용자의 체력이 hp가 0보다 크면 무한 반복  (추가한 부분 )
